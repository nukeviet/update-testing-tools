...
MODULEREPOURL="https://github.com/nukeviet/module-wallet.git" # Repo chứa module để test
UPDATEREPOURL="https://github.com/nukeviet/module-wallet.git" # Repo chứa gói cập nhật để test

# Cho phép truyền tham số đầu tiên là tên phiên bản để chạy từ đó trở đi, ví dụ: bash wallet-4.6.sh 4.5.00
START_VERSION="$1"
START_INDEX=0
...
for i in "${!VERSIONS[@]}"; do
  ...
  php $DIR_PATH/vendor/bin/codecept run -g install-module-wallet
  ...
  php $DIR_PATH/vendor/bin/codecept run -g verify-wallet
  code=$?
  if [[ $code -gt 0 ]]; then
    echo "Tests failed with code: $code on version $version_name"
    read -p "Error! Press any key to continue..."
